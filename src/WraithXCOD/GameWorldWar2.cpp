...
bool GameWorldWar2::LoadOffsets()
{
	...
	if (CoDAssets::GameInstance != nullptr)
	{
		...
		// Attempt to locate via heuristic searching (TODO: Fix... WWII MP is broken...) We can probably track an easier ptr

		// TODO: Fix heuristic for WWII MP NOWZ
		auto DBAssetsScan = (CoDAssets::GameFlags == SupportedGameFlags::SP) ? CoDAssets::GameInstance->Scan("44 0F B7 C7 8B D6 FF ?? 48 8B D8 48 85 C0") : 1;
		...
		if (DBAssetsScan > 0 && StringTableScan > 0 && PackagesTableScan > 0)
		{
			...
			auto GameOffsets = DBGameInfo(
				...
				CoDAssets::GameInstance->Read<uint32_t>(DBAssetsScan + 0x44) + BaseAddress,
